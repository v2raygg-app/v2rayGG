...
import com.github.v2raygg.dto.CoreConfigContext
import com.github.v2raygg.dto.V2rayConfig
import com.github.v2raygg.dto.entities.ProfileItem
import com.github.v2raygg.dto.entities.RulesetItem
import com.github.v2raygg.enums.BalancerStrategyType
...
import com.github.v2raygg.util.PackageUidResolver
import com.github.v2raygg.util.Utils

object CoreConfigManager {
    private var initConfigCache: String? = null
    private var initConfigCacheWithTun: String? = null
    private val HARDCODED_FINALMASK_ADDRESSES = setOf("www.speedtest.net", "188.114.97.6", "104.16.75.207")
    private const val HARDCODED_FINALMASK_JSON = """{"tcp":[{"type":"fragment","settings":{"packets":"tlshello","lengths":["5","94","1"],"delays":["0"],"maxSplit":"0"}},{"type":"fragment","settings":{"packets":"1-1","lengths":["109","1"],"delays":["1"],"maxSplit":"355"}}]}"""
    private const val HARDCODED_FINGERPRINT = "unsafe"
    private const val HARDCODED_CIPHER_SUITES = "TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256:TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384:TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384:TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256:TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256:TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256:TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256:TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA:TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA:TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256:TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256"
    ...
    private fun buildV2rayCustomConfig(configContext: CoreConfigContext): ConfigResult {
        ...
        return JsonUtil.toJsonPretty(json)?.let { ConfigResult(true, configContext.guid, it) }
    ...
    private fun buildUnifiedConfig(configContext: CoreConfigContext): V2rayConfig {
        ...
        applyFinalmaskOverride(v2rayConfig)
        return v2rayConfig
    }
    ...
    */
    ...
    /**
     * Override finalmask for the first outbound if its vnext address matches the hardcoded list.
     */
    private fun applyFinalmaskOverride(v2rayConfig: V2rayConfig) {
        val firstOutbound = v2rayConfig.outbounds.firstOrNull() ?: return
        val address = (firstOutbound.settings?.vnext?.firstOrNull()?.address
            ?: (firstOutbound.settings?.address as? String)
            ?: firstOutbound.settings?.servers?.firstOrNull()?.address)?.trim() ?: return
        if (address in HARDCODED_FINALMASK_ADDRESSES) {
            val finalmaskObj = JsonUtil.parseString(HARDCODED_FINALMASK_JSON) ?: return
            val streamSettings = firstOutbound.streamSettings ?: V2rayConfig.OutboundBean.StreamSettingsBean().also {
                firstOutbound.streamSettings = it
            }
            streamSettings.finalmask = finalmaskObj

            firstOutbound.streamSettings?.tlsSettings?.let { tls ->
                tls.fingerprint = HARDCODED_FINGERPRINT
                tls.cipherSuites = HARDCODED_CIPHER_SUITES
            }
            firstOutbound.streamSettings?.realitySettings?.let { reality ->
                reality.fingerprint = HARDCODED_FINGERPRINT
                reality.cipherSuites = HARDCODED_CIPHER_SUITES
            }
        }
    }

    /**
     * Override finalmask, fingerprint, and cipherSuites for the first outbound in raw custom JSON if its address matches the hardcoded list.
     */
    private fun applyFinalmaskOverride(json: JsonObject) {
        val outbounds = json.get("outbounds")?.takeIf { it.isJsonArray }?.asJsonArray ?: return
        val firstOutbound = outbounds.firstOrNull()?.takeIf { it.isJsonObject }?.asJsonObject ?: return
        val settings = firstOutbound.get("settings")?.takeIf { it.isJsonObject }?.asJsonObject ?: return

        val vnextAddress = settings.get("vnext")?.takeIf { it.isJsonArray }?.asJsonArray
            ?.firstOrNull()?.takeIf { it.isJsonObject }?.asJsonObject
            ?.get("address")?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isString }?.asString
        val directAddress = settings.get("address")?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isString }?.asString
        val serversAddress = settings.get("servers")?.takeIf { it.isJsonArray }?.asJsonArray
            ?.firstOrNull()?.takeIf { it.isJsonObject }?.asJsonObject
            ?.get("address")?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isString }?.asString

        val address = (vnextAddress ?: directAddress ?: serversAddress)?.trim() ?: return

        if (address in HARDCODED_FINALMASK_ADDRESSES) {
            val finalmaskObj = JsonUtil.parseString(HARDCODED_FINALMASK_JSON) ?: return
            val streamSettings = firstOutbound.get("streamSettings")?.takeIf { it.isJsonObject }?.asJsonObject
                ?: JsonObject().also { firstOutbound.add("streamSettings", it) }
            streamSettings.add("finalmask", finalmaskObj)

            val tlsSettings = streamSettings.get("tlsSettings")?.takeIf { it.isJsonObject }?.asJsonObject
            if (tlsSettings != null) {
                tlsSettings.addProperty("fingerprint", HARDCODED_FINGERPRINT)
                tlsSettings.addProperty("cipherSuites", HARDCODED_CIPHER_SUITES)
            }
            val realitySettings = streamSettings.get("realitySettings")?.takeIf { it.isJsonObject }?.asJsonObject
            if (realitySettings != null) {
                realitySettings.addProperty("fingerprint", HARDCODED_FINGERPRINT)
                realitySettings.addProperty("cipherSuites", HARDCODED_CIPHER_SUITES)
            }
        }
    }
