...
#    com.github.v2raygg.service.TProxyService for the VpnService hev tun mode.
...
"$NDK_HOME/ndk-build" \
    ...
    NDK_OUT="$TMPDIR/obj" \
    "APP_CFLAGS=-O3 -DPKGNAME=com/github/v2raygg/service" \
...
#    process by com.github.v2raygg.core.root for the Root run mode. Same hev source,
...
# Sync to V2rayNG/app/libs if present
if [[ -d "$__dir/V2rayNG/app/libs" ]]; then
  for abi in $ABIS; do
    mkdir -p "$__dir/V2rayNG/app/libs/$abi"
    cp "$__dir/libs/$abi/libhev-socks5-tunnel.so" "$__dir/V2rayNG/app/libs/$abi/"
    cp "$__dir/libs/$abi/libhevsockstun.so" "$__dir/V2rayNG/app/libs/$abi/"
  done
fi
